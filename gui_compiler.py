...
class CompilerDialog:
    ...
    def _finalize_scan(self, groups: List[CompilationGroup]):
        ...
        # Исправить строки, добавленные как compile до post-pass, но ставшие cleanup.
        _compile_tags = {'ok', 'warn', 'alpha', 'overlap'}
        for g in cleanup_groups:
            iid = str(id(g))
            if self._tree.exists(iid) and set(self._tree.item(iid, 'tags')) & _compile_tags:
                dup_count = len(g.duplicate_paths or [])
                if g.kept_paths:
                    _order = '✓ Уже скомпилировано'
                else:
                    _p = g.series.rsplit('\\', 1)[0] if '\\' in g.series else g.series
                    _order = f'⊘ Перекрыто: {_p}'
                self._tree.item(iid, tags=('cleanup',),
                                values=(g.author, g.series,
                                        f'0 (удалить: {dup_count})',
                                        _order, g.volume_range or '—'))
    ...
    def _on_select(self, _event=None):
        ...
        try:
            ...
            _arc_part_count = 0
            def _is_arc_unit_gui(b):
                lo, hi = self._service._precompiled_range(b, group.series)
                if lo == hi > 0:
                    return True
                return (b.sort_key[0] == 0 and b.sort_key[1] > 0
                        and b.sort_key[2] == 0)
            _all_arc_point = bool(group.books) and all(_is_arc_unit_gui(b) for b in group.books)
            if _all_arc_point:
                ...
                for b in group.books:
                    _lo, _hi = self._service._precompiled_range(b, group.series)
                    if _lo == _hi > 0:
