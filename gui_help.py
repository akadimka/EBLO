...
Приоритет источников автора (от высшего к низшему):
  ...
  1. folder_dataset — автор определён из структуры папок через
                      folder_author_parser (самый надёжный источник)
  2. filename       — извлечён из имени файла по паттернам
  3. metadata       — взят из FB2 <author> (наименее надёжный,
                      часто пустой или неправильный)
  4. collection     — файл имеет ≥ 3 авторов → "Сборник"
  5. consensus      — усреднение по папке (Pass 4)
  6. (пусто)        — автор не найден

  Источник хранится в BookRecord.author_source и в БД.
"""),
    ("Алгоритм pipeline", "Определение серии", """\
Алгоритм поиска серии:

  1. Из <sequence name="..."> в FB2 → metadata_series + series_number
  2. Из имени файла по паттернам (Pass 2 Series Filename):
       "Автор - Серия 1. Название.fb2"
       "Автор. Серия. Название.fb2"
  3. Из имени папки (folder_series_parser):
       "Автор Имя (Серия 1-3)" и т.п.
