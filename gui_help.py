...
"""),
    ("Меню", "Действия", """\
  • Сканирование    — просканировать FB2-файлы выбранной папки,
                      извлечь жанры из каждого файла и отобразить
                      в режиме списков (Жанры | Ошибки | Детали).
                      Клик по строке «Жанры» → список файлов с этим жанром.

  • Нормализация    — открыть окно просмотра и редактирования результатов
                      (таблица с фильтром, кнопки: Создать CSV, Получить имена,
                       Битые файлы, Великомученницы, Дубликаты,
                       Удалить пустые папки, Скомпилировать).

  • Синхронизация   — переложить файлы по целевой структуре папок,
                      обновить FB2-теги и записать данные в БД.

  • Заархивировать  — упаковать каждый FB2-файл в ПАПКЕ БИБЛИОТЕКИ
                      в zip-архив (file.fb2 → file.fb2.zip).
                      Внутри архива файл хранится под своим именем —
                      стандарт, совместимый со всеми FB2-читалками.
                      Оригинал удаляется после успешной упаковки.
...
Каждый PASS — отдельный класс в папке passes/.
  ...
  pass2_filename.py       — извлечение автора из имени файла:
                            ...
                            Раскрытие инициалов (_expand_initial_surnames):
                              ...
                              источник = filename+meta_expanded.
...
"""),
    ("Модули", "samlib_client.py", """\
Рейтинги Samlib.ru (SamlibWindow).

  Сайт не имеет API — используется парсинг HTML (кодировка cp1251).

  Поиск автора: POST https://samlib.ru/cgi-bin/seek (FIND=<имя>)
    → список авторов со ссылками вида /{буква}/{slug}/

  Книги автора: GET https://samlib.ru/{slug}/indexvote.shtml
    → список книг с рейтингом в формате Оценка:<b>7.31*549</b>
    (оценка * количество голосов).

  Интерфейс: левая панель — список авторов, правая — книги с сортировкой
  по оценке/голосам. Фильтр по названию. Кнопка «Открыть на Samlib.ru».
  Кнопка «Рейтинг (Samlib)» в SearchWindow открывает окно с авто-поиском.
"""),
...
"""),
    ("Модули", "fb2_compiler.py", """\
Сервис компиляции серий (класс FB2CompilerService).

  find_groups(records, work_dir)
      Группирует BookRecord по (author.lower(), series.lower()),
      возвращает группы с ≥2 книгами.

  Порядок книг (многоуровневый):
    1. series_number — целое число из метаданных
    ...
    5. Порядок не определён → группа пропускается

  compile_group(group, output_dir, delete_sources)
      Собирает один FB2 из нескольких:
        — Каждая книга как отдельный <body> с <title><p>N. Название</p></title>
        — Имя файла: Автор - Серия (ДилогияТрилогия|1-17).fb2
        — 2 книги → Дилогия, 3 → Трилогия … 10 → Декалогия, >10 → диапазон
      Частичные компиляции (1-3 + 4-7) → объединяются в одну (1-7).
"""),
    ("Модули", "gui_compiler.py", """\
Диалог компиляции серий (класс CompilerDialog).

  Открывается из gui_normalizer.py кнопкой «Скомпилировать».
  Работает исключительно с записями уже загруженной таблицы.

  • Таблица групп с цветовой индикацией:
      зелёный  — порядок книг определён
      жёлтый   — порядок не определён, группа будет пропущена
  • Детальный список книг выбранной группы (источник сортировки)
  • Выбор папки результата (по умолчанию = work_dir)
  • Чекбокс «Удалять исходники сразу» — иначе запрос после завершения
  • Фоновое выполнение, итоговый отчёт
"""),
    ("Модули", "genre_assign.py", """\
Сервис присвоения жанра FB2-файлам (класс GenreAssignmentService).

  assign_genre_to_folder(folder_path, genre_name, ...)
      Рекурсивно (rglob) ищет *.fb2 в папке и её подпапках,
      удаляет все существующие <genre> и вставляет один новый перед </title-info>.
      Поддерживает ZIP-упакованные FB2 (читает, но не перезаписывает архив).
      Сохраняет оригинальную кодировку файла (CP1251 / UTF-8).

  assign_genre_threaded(...) — обёртка в daemon-Thread.
"""),
    ("Модули", "gui_main.py — Сканирование", """\
Пункт меню «Действия → Сканирование».

  Запускает фоновый поток, который:
    1. Создаёт FB2AuthorExtractor
    2. rglob('*.fb2') по выбранной папке
    3. Вызывает _extract_genres_from_fb2() для каждого файла
    4. Группирует файлы по уникальным наборам жанров
    5. Переключает главное окно в режим «listboxes»:
         Жанры | Ошибки | Детали
    Клик по строке «Жанры» → список файлов в «Детали».
"""),
    ("Модули", "gui_main.py — Заархивировать", """\
Пункт меню «Действия → Заархивировать».

  Работает ТОЛЬКО с папкой библиотеки (settings.get_library_path()).
  Для каждого *.fb2:
    1. Создаёт file.fb2.zip (ZIP_DEFLATED, уровень 6)
    2. Внутри архива файл хранится под именем без пути (arcname=fb2_path.name)
    3. Удаляет оригинал
  Стандарт fb2.zip совместим с PocketBook, Marvin, Kybook и др.
  Фоновый поток, прогресс каждые 50 файлов.
"""),
...
Приоритет источников автора (от высшего к низшему):
  ...
  4. filename+meta_expanded   — автор извлечён из имени файла, но инициалы
