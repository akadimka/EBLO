...
class Pass2SeriesFilename:
    ...
    def execute(self, records: List[BookRecord]) -> None:
        ...
        # 🔑 МНОГОАВТОРНЫЕ СЕРИЙНЫЕ ПАПКИ: "СерияНазвание (Фамилия и др)"
        # Обрабатываем после основного цикла, когда proposed_author уже нормализован.
        self._handle_multiauthor_series_folders(records)
    ...
    def _handle_multiauthor_series_folders(self, records: List[BookRecord]) -> None:
        """Обработка папок формата 'СерияНазвание (Фамилия и др)'.

        Паттерн: корневая папка вида "МоёНазвание (Красницкий и др)" указывает на
        многоавторную серию, где "МоёНазвание" — название серии, "Красницкий" — главный автор.

        Результат:
        - proposed_series = "МоёНазвание" или "МоёНазвание\\Подсерия" (из пронумерованных подпапок)
        - proposed_author = "Красницкий Евгений и другие" (полное имя из метаданных + хинт)

        Не трогает:
        - Одиночные книги в корне папки с одним уникальным автором (не в хинте)
        - Записи с author_source='filename' где автор явно другой человек
        """
        _MA_FOLDER_RE = re.compile(
            r'^(.+?)\s*\(([^)]*(?:и\s+др\.?|и\s+другие|et\s+al\.?|and\s+others)[^)]*)\)\s*$',
            re.IGNORECASE | re.UNICODE,
        )
        _ET_AL_CLEAN = re.compile(
            r'\s*,?\s*(?:и\s+др\.?|и\s+другие|et\s+al\.?|and\s+others)\s*$',
            re.IGNORECASE,
        )
        _NUM_PREFIX_RE = re.compile(r'^\d+[\.\)\-]\s*')

        # Шаг 1: найти многоавторные папки и привязать к ним записи
        # ma_folders: folder_name → {series_name, hint_words, entries: [(record, subfolder|None)]}
        ma_folders: dict = {}

        for record in records:
            parts = Path(record.file_path).parts  # все части пути включая файл
            for idx, part in enumerate(parts[:-1]):   # все папки без самого файла
                m = _MA_FOLDER_RE.match(part)
                if not m:
                    continue
                series_name = m.group(1).strip()
                author_hint_raw = _ET_AL_CLEAN.sub('', m.group(2)).strip().strip(',').strip()
                hint_words = {w.lower().replace('ё', 'е') for w in author_hint_raw.split() if len(w) > 2}

                if part not in ma_folders:
                    ma_folders[part] = {
                ...
                # Подпапка между ma_folder и файлом (если есть)
                subfolder = None
                if idx + 1 < len(parts) - 1:
                    sub_raw = parts[idx + 1]
                    if not sub_raw.lower().endswith('.fb2'):
                        subfolder = _NUM_PREFIX_RE.sub('', sub_raw).strip()

                ma_folders[part]['entries'].append((record, subfolder))
                break  # берём самую высокую совпавшую папку

        if not ma_folders:
            return

        # Шаг 2: для каждой ma_folder определить canonical_author
        for folder_name, info in ma_folders.items():
            series_name = info['series_name']
            hint_words = info['hint_words']
            entries = info['entries']

            # Ищем нормализованное имя: в proposed_author находим КОНКРЕТНОГО автора
            # чьё имя содержит слово из хинта (proposed_author уже нормализован pipeline'ом)
            canonical_author = None
            for record, _ in entries:
                for pa_part in re.split(r'\s*,\s*', record.proposed_author or ''):
                    pa_lower = pa_part.strip().lower().replace('ё', 'е')
                    if any(hw in pa_lower for hw in hint_words):
                        canonical_author = pa_part.strip()
                        break
                if canonical_author:
                    break

            if not canonical_author:
                # Fallback: поискать в metadata_authors
                for record, _ in entries:
                    if not record.metadata_authors:
                        continue
                    for meta_a in re.split(r'[;,]', record.metadata_authors):
                        meta_a = meta_a.strip()
                        meta_words = {w.lower().replace('ё', 'е') for w in meta_a.split() if len(w) > 2}
                        if hint_words & meta_words:
                            canonical_author = meta_a  # сырое имя, нормализуем ниже
                            break
                    if canonical_author:
                        break

            if not canonical_author:
                canonical_author = ' '.join(w.capitalize() for w in hint_words)

            canonical_with_others = f"{canonical_author} и другие"

            # Шаг 3: применяем к записям
            applied = 0
            _COLLECTIVE = {'соавторство', 'сборник', '[unknown]', 'коллектив авторов', 'соавторство'}
            for record, subfolder in entries:
                # НЕ переопределяем одиночные книги с явно другим автором (Клюкин, Al1618 и т.п.)
                # Признак: один реальный автор в метаданных И он не входит в hint_words
                meta_authors_list = [
                    a.strip() for a in re.split(r'[;]', record.metadata_authors or '') if a.strip()
                ]
                if len(meta_authors_list) == 1:
                    single_meta = meta_authors_list[0].lower().replace('ё', 'е')
                    # Коллективные термины — пропускаем фильтр (они "многоавторные")
                    is_collective = single_meta in _COLLECTIVE
                    if not is_collective:
                        meta_words = {w for w in single_meta.split() if len(w) > 2}
                        if meta_words and not (meta_words & hint_words):
                            # Одиночный реальный автор, не из серии → пропускаем
                            continue

                # Автор
                record.proposed_author = canonical_with_others
            ...
            if applied:
                self.logger.log(
                    f"[PASS 2 Series] Multiauthor folder '{folder_name}': "
                    f"author='{canonical_with_others}', series='{series_name}', applied to {applied} records"
                )
