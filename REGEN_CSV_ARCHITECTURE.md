...
## 🔧 ИСПРАВЛЕНИЕ БАГОВ (Февраль 20, 2026)
...
"Земляной Андрей; Живой Алексей" → "Земляной Андрей, Живой Алексей" (нормализовано)

Соавторы (разделитель ', ' из имён файлов) - ❌ ДО ФИКСА:
"Дмитрий Зурков, Игорь Черепнев" → ПОТЕРЯ ВТОРОГО АВТОРА (баг)

Соавторы (разделитель ', ' из имён файлов) - ✅ ПОСЛЕ ФИКСА:
"Дмитрий Зурков, Игорь Черепнев" → "Зурков Дмитрий, Черепнев Игорь" (оба сохранены)
```

**Баг-фикс (Февраль 20, 2026):** 

Метод `normalize_format()` теперь корректно обрабатывает ОБА разделителя:
- `;` (точка с запятой) - из FB2 метаданных
- `,` (запятая) - из извлечения имён файлов (PASS 2)

Ранее метод игнорировал разделитель `,` и обрабатывал весь строковый как одиночное имя, что приводило к потере соавторов.
