# Архитектура системы регенерации CSV (Версия 4.7)

**📌 Дополнительная документация по поддержке соавторства (Co-authorship):** см. [COAUTHORSHIP_FEATURE.md](COAUTHORSHIP_FEATURE.md)

---

## 🚀 Апрель 6–7, 2026 — КАЧЕСТВО СЕРИЙ, ОТКЛОНЕНИЕ МУСОРА, ТОЧНОСТЬ ИСТОЧНИКОВ

---

### 1️⃣ Два новых значения `series_source`

| Новый источник | Смысл |
|---|---|
| `filename+meta_confirmed` | Серия извлечена из имени файла **и** подтверждена тегом `<sequence>` в FB2 |
| `folder_metadata_confirmed` | Серия извлечена из имени **папки**, но уточнена метаданными (например, сокращённое название → полное) |

**Почему переименование:** старое значение `metadata_folder_confirmed` для серий ложно указывало на то, что метаданные первичны. Теперь `folder_metadata_confirmed` однозначно читается как «папка первична, метаданные уточняют». Для `author_source` значение `metadata_folder_confirmed` сохранено (там метаданные действительно первичны).

**`filename+meta_confirmed`** выставляется в двух точках `pass2_series_filename.py` сразу после `record.series_source = "filename"`, если `metadata_series.strip().lower() == clean.lower()`.

**Обновлённая шкала всех источников:**

| Источник | Применимость | Значение |
|---|---|---|
| `folder_dataset` | автор + серия | Папка — прямой источник (Pass 1 / `_propagate_ancestor_folder_authors`) |
| `folder_hierarchy` | серия | Папка переопределила или подтвердила серию через иерархию |
| `folder_metadata_confirmed` | серия | Серия из папки, уточнённая метаданными |
| `metadata_folder_confirmed` | автор | Метаданные автора подтверждены совпадением с папкой |
| `filename+meta_confirmed` | серия | Серия из имени файла, подтверждённая тегом `<sequence>` |
| `filename_meta_confirmed` | автор | Инициалы из имени файла раскрыты через метаданные |
| `filename` | автор + серия | Только из имени файла |
| `metadata` | автор + серия | Только из FB2-тегов |
| `collection` | автор | Определено как Сборник или Соавторство |

**Приоритет серий (убывание):** `folder_dataset` = `folder_hierarchy` > `folder_metadata_confirmed` > `filename+meta_confirmed` > `filename` > `metadata`

---

### 2️⃣ Очистка двоеточий в именах авторов и сериях

- **Файлы:** `passes/pass3_normalize.py`, `passes/pass3_series_normalize.py`
- **Проблема:** FB2-файлы с китайскими метаданными хранят поле автора в формате `作者：牛顿不秃顶`, где `作者` означает «Автор», а `：` — полноширокое двоеточие U+FF1A. После извлечения в `proposed_author` попадало всё поле целиком с меткой.
- **Решение:**
  - В `pass3_normalize.py`: если в `proposed_author` есть `：` или `:`, берётся часть **после** первого двоеточия (`split(colon, 1)[1].strip()`).
  - В `pass3_series_normalize.py` (шаг 1.1 `_normalize_series_name`): аналогичная логика с защитой: если перед двоеточием стоит 1–2 слова с заглавной буквы (имя-префикс типа `Байши Сюсянь:`), берётся часть после. Иначе двоеточие просто удаляется, чтобы не ломать легитимные названия вроде `Война: год первый`.
- **Результат:** `作者：牛顿不秃顶` → `牛顿不秃顶`; `Байши Сюсянь: Я могу исправить свой талант` → `Я могу исправить свой талант` ✅

---

### 3️⃣ Приоритет при частичном совпадении «папка + метаданные»

- **Файл:** `passes/pass2_series_filename.py`, метод `execute()`
- **Проблема:** Для папки `Ацтек (RedDetonator)` функция `_extract_series_from_folder_name` возвращала `"Ацтек"` (скобка с автором отсечена). Метаданные содержали `"Ацтек (RedDetonator)"`. Старый код проверял `meta_l.startswith(folder_l)` = True и выбирал **мету** — с именем автора в скобках.
- **Решение:** Инвертирована логика выбора:
  - Если **мета длиннее папки** (`meta_l.startswith(folder_l) and len(meta_l) > len(folder_l)`) → мета содержит лишнее → берём очищенное имя из папки (`"Ацтек"`).
  - Если **папка длиннее меты** → папка добавляет описание, мета точнее → берём мету (поведение как раньше для случая «Барраяр» vs «Барраярский цикл»).
- **Результат:** `Ацтек (RedDetonator)` → серия `"Ацтек"` ✅

---

### 4️⃣ Три новых класса отклонения в `_is_valid_series`

- **Файл:** `passes/pass2_series_filename.py`, метод `_is_valid_series()`

#### а) Перечень жанров через запятую
- **Пример:** `"Путешествия, приключения, фантастика"` — издательская рубрика, не серия.
- **Правило:** Если строка содержит `,`, все части через запятую ≤ 2 слова, и хотя бы одна часть целиком совпадает с blacklist-словом → отклонить (ПРОВЕРКА -1 перед основным blacklist-циклом).

#### б) Название литературной премии
- **Пример:** `"Литературная премия «Электронная буква – 2019»"` — метаданные некоторых FB2 содержат название премии в поле `<sequence>`.
- **Правило:** Содержит `премия / award / prize / лауреат / номинант` ИЛИ заканчивается на `– YYYY` / `— YYYY` (год с тире) → отклонить.

#### в) Иерархия с однобуквенным сегментом
- **Пример:** `"Р\или Сказ о том как Михалыч пить бросал"` — артефакт разбора аббревиатуры `О. Р. З.`
- **Правило:** Если строка содержит `\` и любой сегмент до/после него ≤ 1 символа → отклонить.

---

### 5️⃣ Порог исключения для blacklist-слова в конце строки повышен

- **Файл:** `passes/pass2_series_filename.py`, метод `_is_valid_series()`
- **Было:** Если blacklist-слово стоит последним и перед ним **есть хотя бы одно слово** → допускаем (исключение «Последний солдат СССР»).
- **Стало:** Исключение срабатывает только если перед blacklist-словом ≥ **2 слова** (`bl_word_index >= 2`).
- **Эффект:** `"Современность. Фантастика"` — лишь 1 слово перед «Фантастика» → теперь отклоняется как жанровая рубрика.

---

### 6️⃣ Полноценная валидация в блоке «последний шанс» метаданных

- **Файл:** `passes/pass2_series_filename.py`, конец метода `execute()`
- **Проблема:** Блок «последний шанс» (когда `proposed_series` пуст и есть `metadata_series`) делал лишь точное совпадение всей строки с blacklist (`== bl.lower()`). Строки вида `"Polaris: Путешествия, приключения, фантастика"` и `"Современность. Фантастика"` не совпадали с отдельным словом blacklist целиком и беспрепятственно попадали в серию.
- **Решение:** Блок теперь выполняет полный пайплайн, как основной:
  1. Word-boundary поиск по blacklist (regex `(?<![а-яёa-z])`).
  2. `_extract_series_from_metadata()` — паттерны очистки (например, `"Серия. Название"` → `"Серия"`).
  3. `_remove_blacklist_words()` — удаление отдельных blacklist-слов.
  4. `_is_valid_series()` — финальная валидация (включая новые проверки п. 4).
  5. `_fix_russian_grammar()` — исправление грамматики.

---

### 7️⃣ Аббревиатуры не образуют иерархию серий

- **Файл:** `passes/pass2_series_filename.py`, метод `_extract_main_series_from_multi_level()`
- **Проблема:** Аббревиатура `О. Р. З.` в имени файла разбивалась по `". "` на `["О", "Р", "З", "или Сказ..."]`. Однобуквенные компоненты добавлялись в иерархию как уровни серии, давая мусорную строку `"О\Р\З\или Сказ..."` или `"Р\или Сказ..."` в зависимости от того, как отработал паттерн.
- **Решение:** Если после удаления числовых суффиксов компонент ≤ 1 символа — это буква аббревиатуры. Функция немедленно возвращает `""`, не добавляя ничего в иерархию.

---

### 8️⃣ BlockLevelPatternMatcher не возвращает сырое значение в обход валидации

- **Файл:** `passes/pass2_series_filename.py`, метод `_extract_series_from_filename()`
- **Проблема:** После вызова `_extract_main_series_from_multi_level(series_from_block)`, если тот возвращал `""`, срабатывал `return series_from_block` — сырое нефильтрованное значение шло в вызывающий код в обход `_is_valid_series`, blacklist и всех других проверок. Для файла `Петракеев Андрей - О. Р. З. или Сказ о том как Михалыч пить бросал.fb2` и паттерна `Author - Series. Title` BlockLevelPatternMatcher выделял хвост аббревиатуры как «серию».
- **Решение:** Оба `return series_from_block` убраны. Если `processed_series` пуст → управление передаётся в ШАГ 2 (старые regex-паттерны). Если и там ничего → серия остаётся пустой.
