# Архитектура системы регенерации CSV (Версия 2.4)
...
3. **Удалить служебные слова в конце (с word boundaries):**
   ```
   "Война и Мир том 1" → "Война и Мир"
   "Альпинист книга 2" → "Альпинист"  ← word boundary предотвращает обрезание 'т' в конце
   ```

4. **Применить conversions из config.json (если определены):**
   ```
   series_conversions: {
