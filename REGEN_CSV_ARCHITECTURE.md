...
RegenCSVService.regenerate():
    ...
    │  └─ Применить метаданные как последнюю надежду
    │     └─ Только если proposed_author всё ещё пуст
    │
...
| PASS 2 FB | Метаданные fallback | — | 0 обновлено | ✅ |
...
### 4.0 PRECACHE Phase: Построение иерархии авторских папок

**Назначение:** Рекурсивное сканирование иерархии файлов ДО PASS 1 для кэширования авторских папок.

**Выполняется:** ДО PASS 1, в методе `_build_author_folder_cache()`

**Алгоритм:**
```
1. Рекурсивно пройти все папки в work_dir до folder_parse_limit уровней
2. Для каждой папки:
   ...
   c) Если папка содержит FB2 файлы И имя парсится как автор → добавить в кэ́ш
3. Результат: self.author_folder_cache = {Path: (author_name, confidence)}
...
Пример резумьтата PRECACHE:
  Path("Books/Гоблин (MeXXanik)") → ("Гоблин MeXXanik", "high")
...
### ⚠️ Архитектурные принципы PASS 1-2-Fallback

**ЖЕЛЕЗНОЕ ПРАВИЛО: Разделение ответственности по источникам**

1. **PASS 1 работает ТОЛЬКО с папкой** (folder_dataset)
...
**Пример: Файл "Трофимов Ерофей - Дитя прибоя (Серия).fb2"**
```
Metadata: "Эльхан Аскеров" (ДРУГОЙ автор!)

PASS 1 (папка):
  - Папка не распознана как авторская → пусто

PASS 2 (filename):
  - Extraction: "Трофимов Ерофей" из имени файла
  - Metadata confirm: "Трофимов Ерофей" ≠ "Эльхан Аскеров" → FALSE
  - ✅ ИСПОЛЬЗУЕМ anyway (независимо от confirmation)
  - Результат: proposed_author="Трофимов Ерофей", author_source="filename"

PASS 2 Fallback:
  - proposed_author уже заполнен → пропускаем

PASS 3-6:
  - Работают с "Трофимов Ерофей" как с обычным автором
  
✅ ИТОГ: Правильный автор "Трофимов Ерофей" (из filename)
         Metadata "Эльхан Аскеров" сохраняется для справки
```
